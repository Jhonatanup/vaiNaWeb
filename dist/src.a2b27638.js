...
function fetchMovies() {
  (0, _axios.default)({
    ...
    var list = response.data.results.map(function (item) {
      return _objectSpread(_objectSpread({}, item), {}, {
        poster_path: "https://image.tmdb.org/t/p/w500".concat(item.poster_path),
        backdrop_path: "https://image.tmdb.org/t/p/w500".concat(item.backdrop_path)
      });
    });
    movies.push.apply(movies, _toConsumableArray(list));
    movies.map(function (info) {
      var popularity = info.popularity,
          ...
          original_tile = info.original_tile,
          overview = info.overview,
          release_date = info.release_date,
          title = info.title,
          video = info.video,
          vote_average = info.vote_average;
      document.querySelector("#app").innerHTML += "\n          <h2>".concat(title, "</h2>\n          <img src=").concat(poster_path, " alt=\"poster\" width=\"300\" height=\"300\"/>\n          <p>").concat(overview, "</>\n          <span>").concat(popularity, "</span>\n\n        ");
      console.log(info);
    });
  }).catch(function (err) {
    console.log(err);
